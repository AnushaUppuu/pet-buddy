...
export const addRemainder=async(req:Request,res:Response):Promise<any>=>{
  const {username,petname}=req.params;
   try{
    const searchUser = await user.findOne({username: username});
    if(searchUser){
      ...
      if(searchPet){
        await pet.updateOne({_id:searchPet._id},{$push:{remainders:req.body}});
...
export const addActivity=async(req:Request,res:Response):Promise<any>=>{
  ...
  try{
    ...
    if(searchUser){
      const searchPet=await pet.findOne({$and:[{name:petname},{owner:searchUser._id}]});
      if(searchPet){
        await pet.updateOne({_id:searchPet._id},{$push:{activity:req.body}});
        res.status(200).send("Remainder added successfully");
      }
      else{
        res.status(201).send("Pet not found");
